...
- `odoo-guidance.md` — the framework reference: module structure, ORM fundamentals, views,
  business logic patterns, security, and (in depth) ORM performance, server resource
  utilization, and debugging/profiling. Framework-level only.
- `findings.md` — TPT's own field notes: real, dated incidents from client/product work.
  Append-only; check here for precedent before assuming a problem is new (currently empty — real
  entries get added as the team hits them, not invented ahead of time).
